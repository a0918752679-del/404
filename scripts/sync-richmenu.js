...
async function line(method, endpoint, body, extraHeaders = {}, base = apiBase) {
...
const open = (pathname, label) => ({ type: 'uri', label, uri: uri(pathname) });
...
const menu = {
  ...
  chatBarText: '萬萬沒想到',
  areas: [
    area(0, 420, 833, 423, open('/#scratch', '立即刮刮樂')),  // 立即刮刮樂
    area(833, 420, 834, 423, open('/#odds', '最新賠率')),      // 最新賠率
    area(1667, 420, 833, 423, open('/#events', '最新活動'))    // 最新活動
  ]
};
...
async function createMenu() {
  ...
  await line('POST', '/v2/bot/richmenu/validate', payload);
  const created = await line('POST', '/v2/bot/richmenu', payload);
  const richMenuId = created.richMenuId;
  const image = fs.readFileSync(menu.image);
  await line('POST', `/v2/bot/richmenu/${richMenuId}/content`, image, { 'Content-Type': 'image/jpeg' }, apiDataBase);
