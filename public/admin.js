  ...
  async loadOdds() {
    try {
      ...
      if (!latest) {
        ...
        box.textContent = '目前尚未上傳最新賠率圖片。'; const timeEl = document.querySelector('#oddsLatestTime'); if (timeEl) timeEl.textContent = '尚未上傳';
        return;
      }
      box.className = 'current-odds';
      const t = new Date(latest.uploadedAt).toLocaleString();
      const timeEl = document.querySelector('#oddsLatestTime');
      if (timeEl) timeEl.textContent = t;
      box.innerHTML = `
        <div style="width:100%">
          <img src="${latest.imageUrl}" alt="最新賠率" />
          <div style="display:flex;justify-content:space-between;gap:10px;align-items:center;flex-wrap:wrap;margin-top:10px">
            ...
            <span class="pill ok">目前顯示中</span>
          </div>
          <small style="color:#aeb8c6">${latest.originalFilename || latest.filename || ''} · ${latest.note || '無備註'}</small>
        </div>`;
    } catch (err) {
