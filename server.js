...
// Serve explicit root routes before the static middleware so Zeabur/proxy health checks never fall through to a platform 404.
app.get(['/', '/index.html'], sendIndex);
app.use(express.static(PUBLIC_DIR, { extensions: ['html'], index: 'index.html' }));

function healthPayload() {
  return { ok: true, service: 'ww-kuji-v8.7', time: now(), port: PORT };
}
...
function readDb() {
  ...
  try {
    ...
    if (!raw) throw new Error('DATA_FILE is empty');
    db = JSON.parse(raw);
  } catch (err) {
    ...
    db = JSON.parse(fs.readFileSync(path.join(__dirname, 'data', 'db.seed.json'), 'utf8'));
    writeDb(db);
  }
  for (const key of ['users', 'products', 'orders', 'payments', 'accounting', 'logs', 'oddsUpdates', 'scratchPrizes', 'scratchDraws']) {
...
function verifyOauthState(req, res, provider) {
  ...
  res.clearCookie(`oauth_${provider}`, SESSION_COOKIE_OPTIONS);
...
function activeScratchPrizes(db) {
  return (db.scratchPrizes || []).filter((p) => p.status === 'active' && Number(p.remaining) > 0 && Number(p.weight) > 0);
}
...
function publicPrize(prize) {
  return {
    ...
    remaining: Number(prize.remaining || 0),
    total: Number(prize.total || 0),
    type: prize.type,
    description: prize.description || ''
  };
}

function pickScratchPrize(db) {
  const pool = activeScratchPrizes(db);
  if (!pool.length) return null;
  const totalWeight = pool.reduce((sum, p) => sum + Number(p.weight || 0), 0);
  const hit = crypto.randomInt(1, totalWeight + 1);
  let acc = 0;
  for (const prize of pool) {
    acc += Number(prize.weight || 0);
    if (hit <= acc) return { prize, randomPoint: hit, totalWeight };
  }
  return { prize: pool[pool.length - 1], randomPoint: hit, totalWeight };
}

function drawHash({ drawId, prizeId, userId, createdAt, nonce }) {
  return crypto.createHash('sha256')
    .update([drawId, prizeId, userId, createdAt, nonce, JWT_SECRET].join('|'))
    .digest('hex');
}
...
app.get('/api/config', (_req, res) => {
  res.json({
    baseUrl: BASE_URL,
    bankAccount: process.env.BANK_ACCOUNT || '匯款資訊準備中，請洽客服確認',
    features: {
...
app.post('/api/auth/logout', (_req, res) => {
  res.clearCookie('ww_session', SESSION_COOKIE_OPTIONS);
  res.json({ ok: true });
});
...
// ---------- Online scratch card ----------
app.get('/api/scratch/odds', (_req, res) => {
  const db = readDb();
  const prizes = (db.scratchPrizes || []).map(publicPrize);
  const active = activeScratchPrizes(db);
  const totalWeight = active.reduce((sum, p) => sum + Number(p.weight || 0), 0);
  res.json({
    fairness: {
      rng: 'server-side crypto.randomInt',
      disclosure: '每次開刮由伺服器加密亂數抽出；獎項、權重、剩餘數與開刮紀錄公開顯示。',
      hashRule: 'SHA-256(drawId|prizeId|userId|createdAt|nonce|serverSecret)'
    },
    totalWeight,
    prizes: prizes.map((p) => ({
      ...p,
      estimatedRate: totalWeight > 0 && p.remaining > 0 ? Number((p.weight / totalWeight * 100).toFixed(4)) : 0
    }))
  });
});

app.get('/api/scratch/ledger', (_req, res) => {
  const db = readDb();
  const draws = (db.scratchDraws || []).slice(0, 50).map((d) => ({
    drawId: d.id,
    at: d.createdAt,
...
app.post('/api/scratch/draw', requireUser, (req, res) => {
  const db = readDb();
  const picked = pickScratchPrize(db);
  if (!picked) return res.status(400).json({ error: '目前獎池已售完或未開放，請稍後再試' });
  const { prize, randomPoint, totalWeight } = picked;
  prize.remaining = Math.max(0, Number(prize.remaining || 0) - 1);
  if (prize.remaining <= 0 && prize.total !== 9999) prize.status = 'soldout';

  if (prize.type === 'points' && Number(prize.points || 0) > 0) {
    const user = db.users.find((u) => u.id === req.user.id);
    if (user) user.points = Number(user.points || 0) + Number(prize.points || 0);
  }

  const createdAt = now();
  const drawId = id('draw');
  const nonce = crypto.randomBytes(16).toString('hex');
  const verificationHash = drawHash({ drawId, prizeId: prize.id, userId: req.user.id, createdAt, nonce });
  const draw = {
    id: drawId,
    userId: req.user.id,
    userName: req.user.name,
    ...
    randomPoint,
    totalWeight,
    noncePublicTail: nonce.slice(-6),
    verificationHash,
    createdAt
  };
  db.scratchDraws.unshift(draw);
  db.scratchDraws = db.scratchDraws.slice(0, 500);
  log(db, '線上刮刮樂開刮', { drawId, prizeId: prize.id, prizeTitle: prize.title, hash: verificationHash }, req.user.id);
  writeDb(db);
  res.json({
    draw: {
      id: draw.id,
      ...
      fairness: { rng: 'crypto.randomInt', randomPoint, totalWeight }
    }
  });
});
...
app.post('/api/admin/logout', (_req, res) => {
  res.clearCookie('ww_admin', SESSION_COOKIE_OPTIONS);
...
app.listen(PORT, '0.0.0.0', () => {
  console.log(`萬萬沒想到 V8.7 running on ${BASE_URL} port ${PORT}`);
});
